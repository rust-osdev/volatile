    ...
    T: ?Sized,
{
    ...
    pub fn borrow(&self) -> VolatileRef<T, A::RestrictShared>
    where
        A: Access,
    {
    ...
    // TODO: Add documentation
    pub fn borrow_mut(&mut self) -> VolatileRef<T, A>
    where
        A: Access,
    {
    ...
    /// })};
    /// ```
    pub unsafe fn map<F, U>(self, f: F) -> VolatileRef<'a, U, A::RestrictShared>
    where
    ...
    pub unsafe fn map_mut<F, U>(self, f: F) -> VolatileRef<'a, U, A>
    where
        F: FnOnce(NonNull<T>) -> NonNull<U>,
        U: ?Sized,
        A: Access,
